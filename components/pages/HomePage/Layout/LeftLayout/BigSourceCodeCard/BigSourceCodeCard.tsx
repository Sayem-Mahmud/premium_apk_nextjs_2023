        ...
        <>
            {item.title &&
                <Link href={`/sourceCode/${item._id}`} className="hover:bg-transparent">
                    ...
                    >
                        <div className='flex flex-col md:flex-row gap-y-4' style={{columnGap:'25px', padding:'0px 20px',width:'100%'}}>
                        <div className='overflow-hidden cursor-pointer rounded w-[100%] md:w-[20%]'>
                            <img src={item.imgSrc} alt="" className='mx-auto my-[10px] bg-gray-400 w-[50%] md:w-52 object-cover scale-[1] hover:scale-[1.1] transition-all duration-300' />
                        </div>
                        <div className='px-4 py-2 w-[100%] md:w-[75%]'>
                            ...
                            {/* <span className="text-xl text-[#00AD7F] font-medium">{item.categories}</span> */}
                                <p title={item.title} className='cursor-pointer text-md mt-[15px] text-[#121212] break-words'>{item.allText  && (!item.allText[0].includes('<img') || !item.allText[0].includes('src=')) && item.allText[0] }</p>
                                <div className='mt-3'>
